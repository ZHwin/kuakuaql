...
- `xmh.js` - 星妈会
- `xmyx.js` - 星妈优选
- ~~`sendNotify.js`~~ - **不需要上传**（青龙自带）
- `utils.js` - 工具函数（如果有）

**重要说明**：
- ✅ 青龙面板自带 `sendNotify.js`，脚本会自动使用
- ❌ 不要上传本仓库的 `sendNotify.js`，避免冲突
- 💡 如果需要本地测试，可以保留本地的 `sendNotify.js`
...
**青龙面板已内置通知功能，直接在环境变量中配置即可！**

在青龙面板 → 环境变量中添加（根据需要选择）：
...
WXPUSHER_UIDS=你的用户ID
```

**说明**：
- 青龙面板的 `sendNotify.js` 支持 20+ 种推送方式
- 配置环境变量后，所有脚本都会自动使用
- 无需修改任何脚本文件
...
**排查步骤**:
1. 检查脚本日志，查看是否有 "通知发送成功" 的提示
2. 确认脚本中 `Notify = 1` 或 `CONFIG.NOTIFY = 1`
3. 检查环境变量中是否配置了推送渠道（如 PUSH_KEY）
4. 确认推送渠道的密钥是否正确

**解决方案**:
- 确保脚本中 `Notify = 1`（默认应该是 1）
- 在环境变量中添加推送配置（如 PUSH_KEY）
- 确保已安装 axios 依赖
- 测试推送渠道是否正常（可以用其他脚本测试）
- 如不需要通知：在脚本中设置 `Notify = 0`
